#pragma once

#define VERTEX_POS_SIZE_2D 2 //x, y

#define VERTEX_POS_SIZE_3D 3   //x, y, z
#define VERTEX_NORMAL_SIZE 3 //x, y, z
#define VERTEX_COLOR_SIZE 4    //r, g, b, a
#define VERTEX_TEXTURE_SIZE 2 //x, y

#define VERTEX_POS_INDEX 0
#define VERTEX_COLOR_INDEX 1
#define VERTEX_NORMAL_INDEX 2
#define VERTEX_TEXTURE_INDEX 2

#define VERTEX_POS_OFFSET 0
#define VERTEX_NORMAL_OFFSET 3
#define VERTEX_COLOR_OFFSET 2
#define VERTEX_TEXTURE_OFFSET 3

